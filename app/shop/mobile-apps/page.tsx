...
import { mobileApps, techStackLabels, TechStack } from "@/data/shop";
...
export default function MobileAppsPage() {
   ...
   const filtered = activeFilter
      ? mobileApps.filter((p) => p.techStack.includes(activeFilter))
      : mobileApps;
      ...
      <div className="px-6 lg:px-20 pt-10 mb-20">
         ...
         <div className="py-2.5">
            ...
            <p className="text-sm mt-2 max-w-xl text-muted-foreground">
               Cross-platform React Native and PWA app templates ideal for
               developers and final year students.
            </p>
         </div>

         {/* Filter chips — dot indicator pattern */}
         <div className="flex flex-wrap gap-4 mt-8">
            ...
            >
               <p className="text-sm flex items-center gap-1">
                  <span
                     className={`transition-all duration-300 ${activeFilter === null ? "text-3xl" : "text-[0px]"
                        }`}
                  >
                     •
                  </span>
            ...
            {allStacks.map((stack) => (
               <button
                  key={stack}
                  onClick={() =>
                     setActiveFilter(stack === activeFilter ? null : stack)
                  }
               ...
               >
                  <p className="text-sm flex items-center gap-1">
                     <span
                        className={`transition-all duration-300 ${activeFilter === stack ? "text-3xl text-foreground" : "text-[0px]"
                           }`}
                     >
                        •
                     </span>
         ...
         {/* Grid */}
         <ProductGrid products={filtered} basePath="/shop/mobile-apps" />
      </div>
   );
}
